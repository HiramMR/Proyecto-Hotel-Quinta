...
function Reveal({ children, delay = 0, direction = 'up', className = '' }: {
  ...
  return (
    <div ref={ref} className={`transition-all duration-700 ease-out ${visible ? 'opacity-100 translate-x-0 translate-y-0' : initial[direction]} ${className}`} style={{ transitionDelay: `${delay}ms` }}>
      {children}
    </div>
  );
}
  ...
  const roomSlides = availableRooms.map(room => ({
    ...
    content: (
      <div className="text-center">
        <h3 className="font-display mb-2 drop-shadow-lg" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.8rem,4vw,3rem)', fontWeight: 400, color: 'var(--cream)' }}>
          {room.title}
        </h3>
        {room.popular && (
          <span className="inline-block px-4 py-1 text-xs uppercase tracking-widest font-semibold mb-3" style={{ backgroundColor: 'var(--copper)', color: '#fff', borderRadius: '2px 10px 2px 10px' }}>
            Popular
          </span>
        )}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {availableRooms.map((room, i) => (
              <Reveal key={room.id} direction="up" delay={i * 100} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.33rem)]">
                <RoomCard
                  title={room.title}
